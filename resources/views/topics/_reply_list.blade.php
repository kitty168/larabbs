    ...
    <li class="media" name="reply{{ $reply->id }}" id="reply{{ $reply->id }}">
      <div class="media-left">
        <a href="{{ route('users.show', [$reply->user_id]) }}">
          <img class="media-object img-thumbnail mr-3" src="{{ $reply->user->avatar }}" alt="{{ $reply->user->name }}" width="48" height="48">
        </a>
      </div>
        ...
        <div class="media-heading mt-0 mb-1 text-secondary">
          ...
          @can('destroy', $reply)
            <span class="meta float-right ">
              <form action="{{ route('replies.destroy', $reply->id) }}" method="post" onsubmit="return confirm('确定要删除此评论吗？');">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
